function [svm_model, energy, c_penalty] = svm_train_validate(train_kernel, train_lab, c_penalty)
...
if c_penalty == 0 
    ...
    for c_lev = -10 : 10   %(-4:8)
        ...
        libsvm_train_options = ['-s 0 -t 4 -c ' num2str(c_param) ' -w-1 ' num2str(neg_w) ' -w1 ' num2str(pos_w) ' -v 3 -q'];
        tmp_acc = svmtrain(train_lab, train_kernel, libsvm_train_options);
        if tmp_acc > acc_best
            c_penalty = c_param;
            acc_best = tmp_acc;
        end
